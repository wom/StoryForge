...
def _create_story_section() -> ConfigSection:
    ...
    section = ConfigSection(
        name="story",
        description="Story generation parameters"
    )

    # Story Length
    section.add_field(ConfigField(
        name="length",
        ...
        ini_comment="Story length options: flash (~100 words), short (~300 words), medium (~600 words), bedtime (~1000 words)"
    ))
    ...
    section.add_field(ConfigField(
        ...
        ini_comment="Target age group options: toddler (1-3 years), preschool (3-5 years), early_reader (5-8 years), middle_grade (8-12 years)"
    ))
...
def _create_output_section() -> ConfigSection:
    ...
    section = ConfigSection(
        name="output",
        description="Output and file handling options"
    )
...
STORYFORGE_SCHEMA = type('StoryForgeSchema', (), {
